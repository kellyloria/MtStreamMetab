...
do.ts <- subset(do.ts,
                datetime > "2021-06-10 12:00:00" & datetime < "2021-12-01 00:00:00",
                Q>0.7) # sensor quality minimum threshold
...
tempcheck2$datetime <- tempcheck2$datetime - c(60*60*(6))
# wtr.ts$datetime <- wtr.ts$datetime - c(60*60*(6))

###clean DO data
ggplot(data = do.ts,aes(x=datetime,y=do.obs)) + geom_line()
describe.ts(do.ts)
do.ts.clean <- trim.outliers(do.ts,width = 12,sd.dev = 4) # usually use 3, but the spikes might be poor.  
ggplot(data = do.ts,aes(x=datetime,y=do.obs)) + geom_line() + 
  geom_line(data=do.ts.clean,aes(x=datetime,y=do.obs),col="red")
do.ts.avg <- aggregate.data(data = do.ts.clean,time.step = 15)
...
qplot(datetime, baro , data = dat1, geom="point") +
  theme(axis.text.x = element_text(angle = 25, vjust = 1.0, hjust = 1.0))+
  scale_x_datetime(breaks = date_breaks("1000 hours"))
...
#  write.csv(x = dat1, file = "/Users/kellyloria/Documents/UNR/MSMmetab/CleanDat/21_GBUInputs.csv", row.names = TRUE)
