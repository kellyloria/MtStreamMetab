...
SPC.ts$datetime <-  (as.POSIXct(round_date(
  as.POSIXct(SPC.ts$datetime, format="%Y-%m-%dT%H:%M:%SZ"), 
  hour, unit="5 minutes")))
...
SPC.tsL$datetime <-  (as.POSIXct(round_date(
  as.POSIXct(SPC.tsL$datetime, format="%Y-%m-%dT%H:%M:%SZ"), 
  hour, unit="5 minutes")))
...
# write.csv(x = dat, file = "/Users/kellyloria/Documents/UNR/MSMmetab/CleanDat/BWL_SPC.csv", row.names = TRUE)
...
# # 2021-09-09
SPC.tss <- subset(SPC.ts,
                    datetime >= '2021-06-30 23:00:00 UTC' & datetime < '2021-09-08 00:00:00 UTC') # sensor quality minimum threshold

range(tempcheck$datetime)

qplot(datetime, SPC, data = tempcheck, geom="point") +
  theme(axis.text.x = element_text(angle = 25, vjust = 1.0, hjust = 1.0))+
  scale_x_datetime(breaks = date_breaks("100 hours"))
...
range(SPC.ts2$datetime)
SPC.ts2s <- subset(SPC.ts2,
                  datetime >= '2022-07-09 18:00:00 UTC' & datetime < '2023-01-01 00:00:00 UTC') # sensor quality minimum threshold

range(SPC.ts2s$datetime)
...
SPCdat_up<- rbind(SPC.tss, SPC.ts2s)
SPCdat_up<- na.omit(SPCdat_up)
range((SPCdat_up$datetime))


tempcheck <- subset(SPCdat_up,
                    datetime > '2021-08-16 00:00:00' & datetime < '2021-08-17 :00:00') # sensor quality minimum threshold
...
# write.csv(x = dat, file = "/Users/kellyloria/Documents/UNR/MSMmetab/CleanDat/BWU_SPC.csv", row.names = TRUE)
